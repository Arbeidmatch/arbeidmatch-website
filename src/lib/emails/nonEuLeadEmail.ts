...
export function buildNonEuLeadEmail(firstName: string): string {
  ...
  const sectionMb = "margin-bottom:20px;";
  const heading = "margin:0 0 10px;font-size:15px;font-weight:600;color:#C9A84C;";

  const section1 = `<div style="${sectionMb}">
<p style="${heading}">What you need to work in Norway legally</p>
...
<li style="margin-bottom:0;font-size:13px;line-height:1.9;color:rgba(255,255,255,0.85);">You usually need a <strong>job offer</strong> before you can apply.</li>
  ...
  const section2 = `<div style="${sectionMb}">
<p style="${heading}">The most common path for non-EU workers</p>
<ul style="margin:0;padding-left:20px;">
...
<li style="margin-bottom:0;font-size:13px;line-height:1.9;color:rgba(255,255,255,0.85);"><strong>Specialist permit</strong> for trades (electricians, welders, and similar)</li>
  ...
  const section3 = `<div style="margin-bottom:20px;padding:20px 24px;border:1px solid rgba(201,168,76,0.4);border-radius:10px;background:rgba(201,168,76,0.08);">
<p style="margin:0 0 8px;font-size:15px;font-weight:600;color:#ffffff;">Want the complete step-by-step guide?</p>
...
<a href="${escapeHtml(ctaUrl)}" style="display:inline-block;max-width:180px;background:#C9A84C;color:#0D1B2A;font-weight:700;font-size:13px;padding:12px 28px;border-radius:8px;text-decoration:none;text-align:center;">Get the Full Guide — 39 EUR</a>
</div>`;

  const body = `<p style="margin:0 0 20px;font-size:13px;line-height:1.8;color:rgba(255,255,255,0.8);">Hi ${name},</p>
<p style="margin:0 0 20px;font-size:13px;line-height:1.8;color:rgba(255,255,255,0.8);">Here is a practical overview to help you prepare before you apply.</p>
${section1}
${section2}
${section3}`;
