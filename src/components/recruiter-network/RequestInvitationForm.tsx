...
export default function RequestInvitationForm() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!gdpr_consent) {
      setError("Please confirm that you have read the privacy notice to continue.");
      return;
    }
        ...
        <div className="flex flex-col gap-6">
          <label className="flex min-h-[44px] cursor-pointer items-start gap-3 rounded-[4px] py-2 pr-2">
            ...
            <span id="gdpr-invite-desc" className="text-[13px] leading-[1.5] text-[rgba(255,255,255,0.60)]">
              I have read the{" "}
              <a href="/privacy" target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-[#C9A84C] underline underline-offset-2 hover:opacity-80">privacy notice</a>, which explains how ArbeidMatch Norge AS stores and uses my information to evaluate my invitation request. *
            </span>
          </label>
