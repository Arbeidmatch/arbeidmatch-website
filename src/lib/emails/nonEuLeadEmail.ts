...
export function buildNonEuLeadEmail(firstName: string): string {
  ...
  const p = "margin:0;font-size:13px;line-height:1.8;color:rgba(255,255,255,0.8);";
  const li = "margin-bottom:8px;font-size:13px;line-height:1.9;color:rgba(255,255,255,0.85);";
  const h2 = "margin:0 0 10px;font-size:15px;font-weight:600;color:#C9A84C;";
  const sectionMb = "margin-bottom:24px;";

  const section1 = `
    <div style="${sectionMb}">
      <h2 style="${h2}">What you need to work in Norway legally</h2>
  ...
  const section2 = `
    <div style="${sectionMb}">
      <h2 style="${h2}">The most common path for non-EU workers</h2>
      <ul style="margin:0;padding-left:20px;">
  ...
  const section3 = `
    <div style="padding:20px 24px;border:1px solid rgba(201,168,76,0.4);border-radius:10px;background:rgba(201,168,76,0.08);">
      ...
      <a href="${ctaUrl}" style="display:inline-block;max-width:180px;background:#C9A84C;color:#0D1B2A;font-weight:700;font-size:13px;padding:12px 28px;border-radius:8px;text-decoration:none;text-align:center;">Get the Full Guide — 39 EUR</a>
    </div>`;

  const body = `
    <p style="${p}margin-bottom:24px;">Hi ${name},</p>
    <p style="${p}margin-bottom:24px;">Here is a practical overview to help you prepare before you apply.</p>
    ${section1}
    ${section2}
    ${section3}`;
