...
export async function GET() {
  ...
  try {
    ...
    /** 7-day average daily signups → estimated daily visitors (no real visitor table). */
    const avgDailyFromWeek = Math.max(0, avgDailySignups * multiplier);
    /** Last 24h run-rate as alternative “daily” signal (scaled to full day). */
    ...
    console.error("[eligibility-candidate-activity]", message);
