...
export default function SourceDisclaimer({
  ...
  const textCls =
    variant === "dark" ? "italic text-white/[0.6]" : "italic text-[#6b7280]";
  ...
  return (
    <p className={`mt-2 leading-[1.5] ${sizeCls} ${textCls} ${className}`.trim()}>
      ...
      <a
        ...
        className={`text-[11px] font-medium underline underline-offset-2 ${variant === "dark" ? "text-gold hover:text-gold-hover" : "text-gold-ink hover:text-navy"}`}
      >
        {sourceLabel}
      </a>
    </p>
