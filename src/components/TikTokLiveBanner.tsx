...
function isTikTokLiveEnabled(): boolean {
  const v = process.env.NEXT_PUBLIC_TIKTOK_LIVE;
  if (!v) return false;
...
/**
 * Manual “we are live on TikTok” strip. Set NEXT_PUBLIC_TIKTOK_LIVE=true to show.
 */
export default function TikTokLiveBanner() {
  if (!isTikTokLiveEnabled()) {
    return null;
  }
