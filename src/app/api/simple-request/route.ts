...
import { candidateRequestLinkLetter } from "@/lib/emails/letters";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (transporter) {
      try {
        const letter = candidateRequestLinkLetter({
          jobSummary: job_summary,
          linkUrl,
          to: email,
          unsubscribeUrl: await unsubscribeUrlFor(email, "simple-request"),
        });
